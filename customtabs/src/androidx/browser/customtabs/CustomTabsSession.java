...
import static androidx.browser.customtabs.CustomTabsIntent.EXTRA_ACTION_BUTTON_BUNDLE;
import static androidx.browser.customtabs.CustomTabsIntent.EXTRA_REMOTEVIEWS;
import static androidx.browser.customtabs.CustomTabsIntent.EXTRA_REMOTEVIEWS_PENDINGINTENT;
import static androidx.browser.customtabs.CustomTabsIntent.EXTRA_REMOTEVIEWS_VIEW_IDS;
import static androidx.browser.customtabs.CustomTabsIntent.KEY_DESCRIPTION;
import static androidx.browser.customtabs.CustomTabsIntent.KEY_ICON;
...
public final class CustomTabsSession {
    ...
    public static CustomTabsSession createMockSessionForTesting(
        ...
        return new CustomTabsSession(
                null, new androidx.browser.customtabs.CustomTabsSessionToken.MockCallback(), componentName);
    }
    ...
    public boolean setActionButton(@NonNull Bitmap icon, @NonNull String description) {
        ...
        metaBundle.putBundle(EXTRA_ACTION_BUTTON_BUNDLE, bundle);
        try {
    ...
    public boolean setSecondaryToolbarViews(@Nullable RemoteViews remoteViews,
        ...
        Bundle bundle = new Bundle();
        bundle.putParcelable(EXTRA_REMOTEVIEWS, remoteViews);
        bundle.putIntArray(EXTRA_REMOTEVIEWS_VIEW_IDS, clickableIDs);
        bundle.putParcelable(EXTRA_REMOTEVIEWS_PENDINGINTENT, pendingIntent);
        try {
    ...
    public boolean setToolbarItem(int id, @NonNull Bitmap icon, @NonNull String description) {
        Bundle bundle = new Bundle();
        bundle.putInt(androidx.browser.customtabs.CustomTabsIntent.KEY_ID, id);
        bundle.putParcelable(androidx.browser.customtabs.CustomTabsIntent.KEY_ICON, icon);
        bundle.putString(androidx.browser.customtabs.CustomTabsIntent.KEY_DESCRIPTION, description);

        Bundle metaBundle = new Bundle();
        metaBundle.putBundle(androidx.browser.customtabs.CustomTabsIntent.EXTRA_ACTION_BUTTON_BUNDLE, bundle);
        try {
            return mService.updateVisuals(mCallback, metaBundle);
        } catch (RemoteException e) {
            return false;
    ...
    public int postMessage(String message, Bundle extras) {
        synchronized (mLock) {
            try {
                ...
                return androidx.browser.customtabs.CustomTabsService.RESULT_FAILURE_REMOTE_ERROR;
            }
        }
    }
    ...
    public boolean validateRelationship(@Relation int relation, @NonNull Uri origin,
        ...
        if (relation < androidx.browser.customtabs.CustomTabsService.RELATION_USE_AS_ORIGIN
                || relation > androidx.browser.customtabs.CustomTabsService.RELATION_HANDLE_ALL_URLS) {
            return false;
        }
        try {
